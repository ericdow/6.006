...
class PriorityQueueHeap:
    ...
    def append(self, key):
        ...
        self.queue.append(key)
        # Swap with parent until invariant is satisfied
        current = len(self.queue)-1
        parent = (current-1)/2
        while (self.queue[current] < self.queue[parent] and parent >= 0):
            tmp = self.queue[current]
            self.queue[current] = self.queue[parent]
