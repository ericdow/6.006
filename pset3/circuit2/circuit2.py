...
class RangeIndexAVL(object):
  ...
  def count(self, first_key, last_key):
    ...
    first_in = self.data.find(first_key) != None
    last_in = self.data.find(last_key) != None
    if (first_in and last_in):
        return last_rank - first_rank + 1
    elif (first_in and not last_in):
        return last_rank - first_rank + 1
    elif (not first_in and not last_in):
