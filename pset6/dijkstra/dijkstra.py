...
class PathFinder(object):
    ...
    def dijkstra(self, weight, nodes, source, destination):
        ...
        """
        # Since the queue hashes on addresses, need to attach
        # data to the nodes
        for node in nodes:
            node.parent = None
        ...
        while len(Q) > 0:
            u = Q.extract_min().node
            if u == destination:
                break
            # Relax vertices adjacent to u
            for v in u.adj:
                if v.key.distance > u.key.distance + distance(u,v):
                    v.key.distance = u.key.distance + distance(u,v)
                    v.parent = u
                    
                    # Update the priority queue
                    Q.decrease_key(v.key)

        # Trace the parent pointers back to the source
        path = []
        p = destination
        while p != source:
            path.append(p)
